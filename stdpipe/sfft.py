...
def _assemble_normal_equations(reference, science, weight, kernel_shape,
                                kernel_poly_order, bg_poly_order,
                                x_norm, y_norm):
...
def _build_kernel_sum_constraint(kernel_shape, kernel_poly_order,
                                 flux_poly_order, x_norm, y_norm):
...
def _reconstruct_model(theta, reference, kernel_shape, kernel_poly_order,
                        bg_poly_order, x_norm, y_norm):
...
def solve(
    ...
    if sci.shape != ref.shape:
        raise ValueError(
            "science and template must have the same shape, got %s vs %s"
            % (sci.shape, ref.shape)
        )
    if sci.ndim != 2:
    ...
    if C_kernel is not None and flux_penalty > 0:
        log("SFFT: kernel-sum constraint: %d equations, flux_poly_order=%d, "
    ...
    for iteration in range(max_iter):
        ...
        if n_good < n_total + 10:
            raise RuntimeError(
                "Too few good pixels (%d) for %d parameters" % (n_good, n_total)
            )
        ...
        model = _reconstruct_model(theta, ref, kernel_shape,
                                    kernel_poly_order, bg_poly_order,
                                    x_norm, y_norm)
        residual = sci - model
        ...
        log("SFFT: iteration %d/%d: n_good=%d, rms=%.4f"
        ...
        if np.any(good_mask) and np.isfinite(final_rms) and final_rms > 0:
            ...
            if robust_sigma > 0:
                ...
                log("SFFT: clipped %d pixels (%.3f%%)"
                    % (n_clipped, 100.0 * n_clipped / n_good))
            else:
                break
    ...
    flux_coeffs = _extract_flux_poly(theta, kernel_shape,
                                      kernel_poly_order, flux_poly_order)

    diff = sci - model
    n_good_final = int(np.sum(weight > 0))

    log("SFFT: done. Final RMS=%.4f, n_good=%d (%.1f%%)"
        % (final_rms, n_good_final, 100.0 * n_good_final / (ny * nx)))
    log("SFFT: flux scale polynomial coeffs: %s" % np.array2string(
        flux_coeffs, precision=4, separator=', '))
...
def evaluate_kernel_at(result, x, y, image_shape):
    ...
    poly_k = _poly_terms_2d(np.array(xn), np.array(yn),
                             result.kernel_poly_order)  # (n_kpoly,)
